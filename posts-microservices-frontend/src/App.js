...
//         const response = await fetch(`http://localhost:8001/api/posts/${post_id}/comments`, {
//             method: "POST",
//             headers: { 'Content-Type': 'application/json' },
//             body: JSON.stringify({ post_id, text: comment })
//         });
...
//                                             <input className="w-100 form-control" onChange={e => comment = e.target.value} />
//                                         </form>
//                                     </div>
...
function App() {
    ...
    useEffect(() => {
        (async () => {
            const response = await fetch('http://localhost:8000/api/posts');
            const content = await response.json();
            setPosts(content.map(post => ({
                ...post,
                comments: post.comments || [] // Ensure comments is initialized
            })));
        })();
    }, []);
    ...
    const createPost = async e => {
        ...
        const createdPost = await res.json();
        setPosts([...posts, { ...createdPost, comments: [] }]); // Initialize comments
        setTitle('');
        setDescription('');
    }
    ...
    const createComment = async (e, post_id) => {
        ...
        setPosts(posts.map(p => {
            if (p.id === post_id) {
                p.comments = p.comments || []; // Initialize if undefined/null
                p.comments.push(createdComment);
            }
            return p;
        }));
